...
int unit_can_move(struct block_list *bl) {
	...
	if (ud->skilltimer != INVALID_TIMER && ud->skill_id != LG_EXEEDBREAK && (!sd || (!pc_checkskill(sd, SA_FREECAST) && !sd->bonus.skillmove) || skill_get_inf2(ud->skill_id)&INF2_GUILD_SKILL))
		return 0; // Prevent moving while casting

	if (DIFF_TICK(ud->canmove_tick, gettick()) > 0)
		return 0;

	if ((sd && (pc_issit(sd) || sd->state.vending || sd->state.buyingstore || (sd->state.block_action & PCBLOCK_MOVE) || sd->special_state.no_walk)) || ud->state.blockedmove)		return 0; // Can't move
