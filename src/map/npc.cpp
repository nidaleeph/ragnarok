...
int npc_touch_areanpc2(struct mob_data *md)
{
	...
	for( i = 0; i < mapdata->npc_num_area; i++ )
	{
		if( mapdata->npc[i]->sc.option&OPTION_INVISIBLE || *mapdata->npc[i]->variable )
			continue;
...
#ifdef STORM_ITEM_DURABILITY
			script_setarray_pc(sd, "@sold_durability", i, (void*)(intptr_t)sd->inventory.u.items_inventory[idx].durability, &key_durability);
#endif
