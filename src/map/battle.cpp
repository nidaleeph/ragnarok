...
#define SKILLMAGIC_INF		(INF2_CHORUS_SKILL|INF2_ENSEMBLE_SKILL|INF2_GUILD_SKILL|INF2_SONG_DANCE|INF2_SPIRIT_SKILL|INF2_TARGET_SELF|INF2_TRAP|INF2_WEDDING_SKILL)
...
static int battle_getskillflag(int skill_id, int skill_lv)
{
	...
	if (skill_get_inf(skill_id) == INF_GROUND_SKILL && !(skill_get_nk(skill_id)&NK_NO_DAMAGE) && !(skill_get_inf2(skill_id)&SKILLMAGIC_INF))
...
void battle_damage(struct block_list *src, struct block_list *target, int64 damage, t_tick delay, uint16 skill_lv, uint16 skill_id, enum damage_lv dmg_lv, unsigned short attack_type, bool additional_effects, t_tick tick, bool isspdamage)
{
	...
	if (sd && target && !isspdamage && skill_id && !sd->state.autocast) {
		...
		if (!success && !status_isdead(target) && skill_get_inf(skill_id) != INF_TARGET_TRAP && skill_get_inf(skill_id) != INF_GROUND_SKILL) {
 ...
 */
int battle_calc_cardfix(int attack_type, struct block_list *src, struct block_list *target, int nk, int rh_ele, int lh_ele, int64 damage, int left, int flag, int flag2){
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
        ...
        if ( ( (( (TBL_MOB*)target )->mob_id == 1288 || ( (TBL_MOB*)target )->mob_id == 1905 ) && !strcmp( mapindex_id2name(sd->mapindex), "ra_koe" ) ) &&
